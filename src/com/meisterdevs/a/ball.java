...
public class ball {
	...
	public static void main(String arg[]) {
		...
		/*
		 * Mat S=new Mat(); S.ones(new
		 * Size(hsv_image.width(),hsv_image.height()),CvType.CV_8UC1); Mat V=new
		 * Mat(); V.ones(new
		 * Size(hsv_image.width(),hsv_image.height()),CvType.CV_8UC1); Mat H=new
		 * Mat(); H.ones(new
		 * Size(hsv_image.width(),hsv_image.height()),CvType.CV_8UC1);
		 */
		Mat distance = new Mat(webcam_image.height(), webcam_image.width(), CvType.CV_8UC1);
		// new Mat();//new
		// Size(webcam_image.width(),webcam_image.height()),CvType.CV_8UC1);
		List<Mat> lhsv = new ArrayList<Mat>(3);
		Mat circles = new Mat(); // No need (and don't know how) to initialize
									// it.
		...
		if (capture.isOpened()) {
			while (true) {
				...
				if (!webcam_image.empty()) {
					...
					// "distance"
					// Ideally we would like to cut the image by hue and then
					// pick just
					// the area where S combined V are largest.
					// Strictly speaking, this would be something like
					// sqrt((255-S)^2+(255-V)^2)>Range
					// But if we want to be "faster" we can do just
					// (255-S)+(255-V)>Range
					// Or otherwise 510-S-V>Range
					// Anyhow, we do the following... Will see how fast it
					// goes...
					...
					Imgproc.HoughCircles(thresholded, circles, Imgproc.CV_HOUGH_GRADIENT, 2, thresholded.height() / 4,
							500, 50, 0, 0);
					...
					Core.putText(webcam_image,
							String.format("(" + String.valueOf(data[0]) + "," + String.valueOf(data[1]) + ","
					...
					if (data2.length > 0) {
						...
						for (int i = 0; i < data2.length; i = i + 3) {
							...
							// Core.ellipse( this, center, new Size(
							// rect.width*0.5, rect.height*0.5), 0, 0, 360, new
							// Scalar( 255, 0, 255 ), 4, 8, 0 );
							Core.ellipse(webcam_image, center, new Size((double) data2[i + 2], (double) data2[i + 2]),
